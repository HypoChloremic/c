...
namespace http {
    ...
    void TcpServer::startListen(){
        ...
        while(true){
            ...
            if(bytesReceived < 0) {
                exitWithError("Failed to read bytes from the client socket connection");
            }
    ...
    void TcpServer::acceptConnection(int &new_socket){
        ...
        if(new_socket < 0){
            std::ostringstream ss;
            ss << "Failed to accept incoming connection from ADDRESS: " << inet_ntoa(m_socketAddress.sin_addr) << " PORT: " << ntohs(m_socketAddress.sin_port);
